...
int msm_hsusb_get_serial_number( char *serial_number )
{
	...
	rc = msm_rpc_call_reply(usb_ep, usb_rpc_ids.get_serial_num,
				&req, sizeof(req),
				&rep, sizeof(rep), 5 * HZ);
	...
	if (rc < 0) {
		pr_err("%s: rpc call failed! error: (%d)"
				"proc id: (%lx)\n",
				__func__, rc,
				usb_rpc_ids.get_serial_num);
		return rc;
	}

	memcpy( serial_number, rep.serial_num, be32_to_cpu(rep.length) );

	pr_info("%s: serial_num: %s\n", __func__, rep.serial_num);
	return rc;
}
EXPORT_SYMBOL(msm_hsusb_get_serial_number);
