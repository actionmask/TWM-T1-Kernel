...
static void usb_switch_composition(unsigned short pid)
{
	...
	for (i = 0; i < ui->num_funcs; i++) {
		struct usb_function_info *fi = ui->func[i];
		if (!fi || !fi->func || !(ui->composition->functions & (1 << i)))
			continue;
		if (fi->func->configure)
...
void usb_function_enable(const char *function, int enable)
{
	...
	for( i = 0; i < ui->pdata->num_compositions; i++ )
	{
		if( ui->pdata->compositions[i].product_id == 0xB003 )
		{
			if( !strcmp(function, "adb") )
			{
				if( enable )
					ui->pdata->compositions[i].functions = 0x07; //0000 0111(mass_storage+modem+adb)
				else
					ui->pdata->compositions[i].functions = 0x03; //0000 0011(mass_storage+modem)

				usb_switch_composition( 0xB003 );
				return;
			}
			else
				break;
...
static void usb_configure_device_descriptor(struct usb_info *ui)
{
	...
	char serial_number_len = 0;
	int rpc_result;

	memset( serial_number, 0x00, 20 );
	rpc_result = msm_hsusb_get_serial_number( serial_number );
	if( rpc_result < 0 )
	{
		pr_info( "%s: get serial number rpc call fail\n", __func__ );
	}
	else if( rpc_result > 0 )
	{
		serial_number_len = strlen( serial_number );
		if( serial_number_len > 0 )
			memcpy( ui->pdata->serial_number, serial_number, (serial_number_len+1) );
	}
	pr_info( "%s: serial number: %s\n", __func__, serial_number );
