...
{
	struct ts *ts;
	ts = container_of(handler, struct ts, early_suspend);
	disable_irq(ts->irq);
}
...
{
	DPRINTK("pressure %d , x %d, y %d",pressure,x,y);
	if (pressure) {
...
static int __devinit ts_probe(struct platform_device *pdev)
{
	...
	input_set_abs_params(input_dev, ABS_X, 80, 940, 4, 0);
	input_set_abs_params(input_dev, ABS_Y, 75, 985, 10, 0);
