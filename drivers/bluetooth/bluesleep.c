...
void bluesleep_sleep_wakeup(void)
{
	...
	{
		BT_DBG("None asleep : ext_wake : %d, host_wake : %d, asleep : %d, txdata : %d, proto: %d", \
...
static int bluepower_read_proc_sysrst(char *page, char **start, off_t offset,
					int count, int *eof, void *data)
{
	*eof = 1;
	return sprintf(page, "sys_reset: %u \n", gpio_get_value(bsi->sys_reset));
}
...
static int __init bluesleep_init(void)
{
	...
	/* read only proc entries */
	if (create_proc_read_entry("sysrst", 0, sleep_dir,
				bluepower_read_proc_sysrst, NULL) == NULL) {
		BT_ERR("Unable to create /proc/%s/hostwake entry", PROC_DIR);
		retval = -ENOMEM;
		goto fail;
	}
